...
class ClientUser extends User {
  ...
  async setBanner(filePathOrUrl) {
    try {
      const bannerBuffer = await this._fetchBanner(filePathOrUrl);
      await this._updateBanner(bannerBuffer);
      console.log("Your bot's banner has been updated successfully.");
    } catch (err) {
      console.error(`An error occurred: ${err.message}`);
    }
  }

  async _fetchBanner(filePathOrUrl) {
  ...
  async _updateBanner(bannerBuffer) {
    const response = await fetch("https://discord.com/api/v9/users/@me", {
      ...
      body: JSON.stringify({
        banner: `data:image/gif;base64,${bannerBuffer.toString("base64")}`
      })
    });
    ...
    if (response.status === 429) {
      ...
      setTimeout(() => this._updateBanner(bannerBuffer), retryAfter);
      return;
    }

    if (!response.ok) {
      const errorText = await response.text();
      throw new Error(`Failed to update banner: ${response.status} ${response.statusText} - ${errorText}`);
    }
  }
