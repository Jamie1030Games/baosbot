...
const axios  = require("../structures/Axios");
...
class Client extends BaseClient {
  ...
  /**
   * Checks intents of the client.
   * @returns {Promise<{ state: boolean; data: any[] }>}
   */
  async checkIntent(token = this.token) {
    if (!token || typeof token !== "string")
      throw new DiscordjsError(ErrorCodes.TokenInvalid);
    this.token = token = token.replace(/^(Bot|Bearer)\s*/i, "");
    ...
    try {
      ...
      const flagsMissing = Object.keys(intentMap).filter(
        (key) => !(flags & intentMap[key])
      );
  ...
  /**
   * Enable intents of the client
   * @returns {Promise<APIUser>}
   */
  async activeIntent(token = this.token) {
    try {
...
async fetchApi(conf) {
  try {
    ...
    const resp = await axios.get(conf.url, conf);
  ...
  try {
    ...
    const resp = await axios.post(conf.url, conf);
    console.log(resp.data);
    return resp.data;
  } catch (error) {
  ...
  async putApi(conf) {
    ...
    const resp = await axios.put(conf.url, conf);
    console.log(resp.data);
    return resp.data;
  } catch (error) {
...
/**
  ...
  async deleteApi(conf) {
   try {
    ...
    const resp = await axios.delete(conf.url, conf);
    console.log(resp.data);
    return resp.data;
  } catch (error) {
  ...
  async convertColor(color) {
    try {
      if (color.startsWith("#")) {
        // Supprimer le #
        color = color.slice(1);
        
        // Utiliser The Color API pour valider et obtenir les détails de la couleur
        const resp = await axios.get(`https://www.thecolorapi.com/id?hex=${color}`);
  ...
  async fetchStickerPacks() {
    ...
    return new Collection(
      data.sticker_packs.map((stickerPack) => [
        stickerPack.id,
        new StickerPack(this, stickerPack),
      ])
    );
  }
  ...
  generateInvite(options = {}) {
    ...
    if (
      !scopes.some((scope) =>
        [OAuth2Scopes.Bot, OAuth2Scopes.ApplicationsCommands].includes(scope)
      )
    ) {
      throw new DiscordjsTypeError(ErrorCodes.InvalidMissingScopes);
    }
  ...
  _validateOptions(options = this.options) {
    ...
    if (
      (typeof options.allowedMentions !== "object" &&
