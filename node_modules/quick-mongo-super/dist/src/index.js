...
const DatabaseError_1 = require("./classes/DatabaseError");
 ...
 * @extends {Emitter}
 */
class Mongo extends Emitter_1.Emitter {
    ready = false;
    options;
    mongoClientOptions;
    mongo;
    database;
    collection;
    _utils = new Utils_1.Utils();
    constructor(options) {
        super();
        if (!options?.connectionURI) {
            throw new DatabaseError_1.DatabaseError(errors_1.default.connection.uri.notSpecified);
        }
        if (typeof options?.connectionURI !== 'string') {
            throw new DatabaseError_1.DatabaseError(errors_1.default.connection.uri.invalid);
        }
        if (options?.collectionName && typeof options?.collectionName !== 'string') {
            throw new DatabaseError_1.DatabaseError(errors_1.default.invalidType('options.collectionName', 'string', options?.collectionName));
        }
        if (options?.dbName && typeof options?.dbName !== 'string') {
            throw new DatabaseError_1.DatabaseError(errors_1.default.invalidType('options.dbName', 'string', options.dbName));
        }
        if (options?.mongoClientOptions && typeof options?.mongoClientOptions !== 'object') {
            throw new DatabaseError_1.DatabaseError(errors_1.default.invalidType('options.mongoClientOptions', 'object', options?.mongoClientOptions));
        }
    ...
    async connect() {
        if (this.ready) {
            throw new DatabaseError_1.DatabaseError(errors_1.default.connection.alreadyConnected);
        }
        ...
        const mongo = await mongoClient.connect().catch((err) => {
            if (err.message.toLowerCase().includes('bad auth')) {
                throw new DatabaseError_1.DatabaseError(errors_1.default.connection.badAuth);
            }
            throw new DatabaseError_1.DatabaseError(errors_1.default.connection.failedToConnect + err);
        });
        if (!mongo) {
            throw new DatabaseError_1.DatabaseError(errors_1.default.connection.connectionFailure);
        }
        this.mongo = mongo;
    ...
    async disconnect() {
        if (!this.ready) {
            throw new DatabaseError_1.DatabaseError(errors_1.default.connection.alreadyDestroyed);
        }
        this.ready = false;
    ...
    async random(key) {
        ...
        if (!key) {
            throw new DatabaseError_1.DatabaseError(errors_1.default.requiredParameterMissing('key'));
        }
        if (!Array.isArray(array)) {
            throw new DatabaseError_1.DatabaseError(errors_1.default.target.notArray + typeof array);
        }
    ...
    */
    ...
    async fetch(key) {
        ...
        if (typeof key !== 'string') {
            throw new DatabaseError_1.DatabaseError(errors_1.default.invalidTypes.key + typeof key);
        }
    ...
    async set(key, value) {
        ...
        if (value == undefined) {
            throw new DatabaseError_1.DatabaseError(errors_1.default.requiredParameterMissing('value'));
        }
        if (typeof value == 'function') {
            throw new DatabaseError_1.DatabaseError(errors_1.default.invalidTypes.functionIsValue);
        }
        const keys = key.split('.');
    ...
    async remove(key) {
        ...
        if (data == null || data == undefined) {
            throw new DatabaseError_1.DatabaseError(errors_1.default.target.empty);
        }
        const keys = key.split('.');
        let database = fetched;
        for (let i = 0; i < keys.length; i++) {
    ...
    async add(key, value) {
        ...
        if (typeof value !== 'number') {
            throw new DatabaseError_1.DatabaseError(errors_1.default.invalidTypes.valueNumber + typeof value);
        }
        if (typeof data !== 'number') {
            throw new DatabaseError_1.DatabaseError(errors_1.default.target.notNumber + typeof data);
        }
